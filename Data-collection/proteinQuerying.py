...
def read_protein_sequences_batch(index, keywords_file):
    ...
    sequence_dict = {}
    df = pd.DataFrame(columns=['protein_group', 'protein', 'sequence'])
    for i, (kws, _) in enumerate(index.items()):
        kw_lst = kws.split('_')
        max_evidence = 6
        max_uniprot_id = ''
        max_sequence = ''
        ...
        sequence_dict[kws] = (max_sequence, max_uniprot_id, max_evidence)
        df.loc[i] = [kws, max_uniprot_id, str(max_sequence)]
